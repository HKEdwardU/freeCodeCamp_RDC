...
echo "$($PSQL "select t.name from games g full join teams t on g.winner_id = t.team_id where year = 2018 and round = 'Final'")"

echo -e "\nList of teams who played in the 2014 'Eighth-Final' round:"
echo "$($PSQL "select t.name from teams t left join games g on t.team_id = g.winner_id or t.team_id = g.opponent_id where g.year = 2014 and round = 'Eighth-Final' order by t.name")"
